...
public class Mandelbrot extends Thread {
    ...
    private static volatile boolean isFinished = false;          // Etat de l'image : Mis a Vrai quand l'image est finis


    public void run(){                      // Thread qui enregistre les images toutes les 100ms
        while (!isFinished) {
            synchronized (this) {
                try {
                    wait(80);                           // 80ms car 100ms prenait trop de temps
                } catch (InterruptedException e) {
                    throw new RuntimeException(e);
                }
            }
            if(count == maxPNG-1) return;
            image.save("pic"+(count<10 ? "00"+count : "0"+count)+".png");
            count++;
        }
    }

    public static void main(String[] args) {
        image.save("pic000"+".png");
