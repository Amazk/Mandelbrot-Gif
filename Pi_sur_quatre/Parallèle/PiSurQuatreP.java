...
public class PiSurQuatreP extends Thread {
    static long nbTirages = 700_000_000;          // Précision du calcul, env 9s
    ...
    public static void main (String[] args) {
        if (args.length>0) {
            try { nbTirages = 1_000_000L * Integer.parseInt(args[0]); }
        ...
        PiSurQuatreP[] T = new PiSurQuatreP[nbThreads];
        for(int i=0; i<nbThreads; i++){
            T[i] = new PiSurQuatreP(nbTirages / nbThreads);
            T[i].start();
        }
        for(int i=0; i<nbThreads; i++){
        ...
        int somme = 0;
        for(int i=0; i<nbThreads; i++) {
            somme += (int) T[i].tiragesDansLeDisque;
        }
